...
$(function(){
    ...
    var Amazon = {
        init: function(){
            ...
            $("#commit-title-keys").click(Amazon.commit_title_keys);
            ...
            $("html").on("click","#onekey-info",Amazon.batch_sku_info);
            $("html").on("click","#commit-set-sale",Amazon.batch_set_sale);
            $("html").on("input","#child-title",Amazon.check_child_title);
            $("html").on("input",".child-keys",Amazon.check_child_key);
            ...
            sku_variation.on("click",".sku-title-keys",Amazon.sku_title_keys);
        ...
        check_child_title: function () {
            var v = $(this).val().trim();
            if (v.length>200){
               $(this).val(v.substring(0,200));
            }else{
                console.log(v.length);
                $(this).parent().find(".title-number").text(v.length);
            }
        },
        check_child_key: function () {
            var v = $(this).val().trim();
            if (v.length>50){
               $(this).val(v.substring(0,50));
            }else{
                $(this).next().find("span").text(v.length);
            }
        },
        batch_sku_info: function () {
            var title = $("#product-title").val().trim(),
                sku_table_tr = $(".table").find("tr"),
                keywords_list = [],
                keywords = $("input[name='keyword']");
            for(var i=0; i<keywords.length; i++){
                keywords_list.push($(keywords[i]).val());
            }
            var info = {"Title": title, "KeyWords": keywords_list};
            sku_table_tr.each(function(i,v){
                $(v).find(".v-title-keys").val(JSON.stringify(info));
            })
        },
        commit_title_keys: function () {
            var flag = Amazon.check_sku_info();
            if(!flag){
               Inform.show("请将红框中的内容补充完成！");
                return 0
            }
            var title = $("#child-title").val().trim(),
                $keys = $(".child-keys"),
                keys = [];
            ...
            console.log({"Title": title, "KeyWords": keys});
            child_info.val(JSON.stringify({"Title": title, "KeyWords": keys}));
            $("#set-title-keys").modal("hide");
        },
        check_sku_info: function () {
            var $input = $("#set-title-keys").find("input"),
                flag = true;
        ...
        sku_title_keys: function () {
            $(".child-keys, #child-title").val("").css("border-color", "");
            $(".key-number, .title-number").text("0");
            child_info = $(this).parent().find(".v-title-keys");
            var base = {"Title": "", "KeyWords": []},
                $sku_modal = $("#set-title-keys");
            if(child_info.val().trim()){
               base = JSON.parse(child_info.val().trim())
            }
            console.log(base);
            $("#child-title").val(base["Title"]);
            $(".title-number").text(base["Title"].length);
            for(var i=0;i<base["KeyWords"].length;i++){
                var item = $(".child-keys").eq(i);
                item.val(base["KeyWords"][i]);
                item.next().find("span").text(base["KeyWords"][i].length)
            }
        ...
        choose_cate: function(){
            ...
            $.ajax({
                url: "/amazon/"+shop_id+"/api/product_type",
                type: "POST",
                data:{
                    child_id: cate_id,
                    root_id: big_cate_id
        ...
        choose_pro_type: function(){
            ...
            if(product_type){
                var data = {};
                data["root_id"] = big_cate_id;
                data["product_type"] = type_flag? product_type:"";
                Amazon.get_specifics(data);
            }else{
                sku_tem.closest(".form-group").hide();
            }
        ...
        get_specifics: function(data){
            $.ajax({
                url: "/amazon/"+shop_id+"/api/get_specifics",
                type: "POST",
                data: data,
                success: function(data){
                    if(data["status"]){
                        ...
                        for(var i in data["attributes"]){
                            if(data["attributes"][i]["name"] == "VariationTheme"){
                                var temps = data["attributes"][i]["values"];
                                sku_tem.empty();
                            ...
                            }else if(data["attributes"][i]["sku"]){
                                var obj  = data["attributes"][i];
        ...
        add_pro: function(obj,str){
            ...
            var label = $("<label/>").attr("class","col-md-2 control-label").attr("data-name",obj.name)
                        .text((obj.display_name||obj.name)+":");
            if(obj.required){
                label.prepend("<span class=\"required\">*</span>");
            }
            var check_area = $("<div/>").attr("class","row");
            var check_content= $("<div/>").attr("class","col-md-10 form-inline");
            if(obj["ShowType"]=="CheckBox"){
                ...
                var add_diy = '<div class="col-md-10 col-md-offset-2 form-inline">'
        ...
        create_table: function(sku_values){
            var frame = '<div class="form-group">'
                ...
                +'<span style="margin:8px">批量设置产品编码(只适用于品牌入驻卖家将产品编码与SKU设置一致):'
            ...
            var sku_attr_str = '<tr class="variation-row"><th style="width: 100px">是否生效</th>{0}{1}<th class="variation-th">价格('+$("#currency").val()+')</th>' +
                ...
                '<th>促销开始日期</th><th>促销结束日期</th>' + '<th class="variation-th">库存(件/个)</th><th style="width: 150px">产品编码(UPC/EAN)</th><th>图片URL</th>' +
                '<th style="width: 200px">SKU编码'
                +'<a class="one-btn-sku" id = "onekey-SKU" href="javascript:void(0)">(一键生成SKU)</a></th></th><th>标题和关键词'
                +'<a id = "onekey-info" href="javascript:void(0)">(与父商品一致)</a>'
                + '</th></tr>';
            var sku_attr = "";
                ...
                if(level==max_level){
                    ...
                    row +=  '<td><input type="text" class="form-control v-price"></td>'
                           ...
                           +'<td><input type="text" class="form-control v-upc"></td>'
                            ...
                            +'<td style="width:268px"><div class="input-group"><input type="text" class="form-control v-title-keys" style="word-break: break-all" readonly="readonly"><div class="input-group-addon btn btn-primary sku-title-keys">设置</div></div></td>'
                            +'</tr>';
                    var sku_attr_str = "";
        ...
        onekey_sku : function(){
            var parent_sku = $("#parent-sku").val().trim(),
                reg = $.isEmptyObject(alias);
            if(!parent_sku){
                Inform.show("请先将产品的ParentSku填写完整！")
            }else{
                var sku_table_tr = $(".table").find("tr");
                sku_table_tr.each(function(i,v){
                    var v_sku = parent_sku;
                    $(v).find(".variation-attr").each(function(m,n){
                        var type = $(n).attr("data-name").toLowerCase(),
                            value = $(n).text();
                        if(reg){
                            v_sku += "-"+value;
                        }else{
                            if(alias[type]){
                                v_sku += alias[type][value.toLowerCase()]||value;
                            }else{
                                v_sku += value;
                            }
                        }
        ...
        submit_feed: function(){
            ...
            var valid_info = Amazon.check_valid();
            if(!valid_info["flag"]){
                Inform.enable();
                Inform.show(valid_info["message"]);
                return 0;
            }
        ...
        save_feed: function(){
            ...
            var product_info = Amazon.get_info();
            var url = "/create/"+shop_id+"/product/draft";
            Inform.disable();
        ...
        get_info: function(){
            ...
            for(var i=0;i<img_div.length;i++){
                if (img_div.eq(i).attr("src")!="/static/image/add.png"){
        ...
        check_valid: function(){
            ...
            if($stocks.length > 0){
                $stocks.each(function(k, v){
                    ...
                    if(value == "" || !int_pattern.test(value)){
                        flag = false;
                        message = "库存值必须为大于等于0的整数";
                        return false
                    }
                })
            }
        ...
        render_page: function(){
            ...
            $.ajax({
                "url": "/create/" + $("#shop-id").val() + "/product/get",
                "type": "POST",
                "dataType": "json",
            ...
            sku_variation.find(".variation-row").each(function(k, v){
                ...
                if(k > 0){
                    ...
                    if(pu.SKU){
                        ...
                        //var title_keys = pu["Info"]?pu["Info"]: {"Title": "", "KeyWords": []};
                        var title_keys = pu["Info"]?pu["Info"]: "";
                        var upc = pu["UPC"];
                        ...
                        if(title_keys){
                            title_keys = JSON.stringify(title_keys);
                        }
                        row.find(".v-title-keys").val(title_keys);
                    }else{
    ...
    var Image = {
        ...
        pic_render:function(){
             ...
             for(var i=pic_src.length;i<9;i++){
                 $(".pic-display").find("img[data-index="+i+"]").attr("src","/static/image/add.png");
             }
            $(".pic-display").show();
            $(".pic-space-area").hide();
            $(".pic-url").hide();
        ...
        web_pic_choose:function(){
            ...
            $(".pic-display").find("img").each(function(k,v){
                if($(v).attr("src")!="/static/image/add.png"){
                    pic_num++;
                }
            });
        ...
        set_main_pic:function(){
            ...
            if(pic_to_set != "/static/image/add.png"){
        ...
        del_pic_mod:function() {
            if ($(this).closest(".thumbnail").find("img").attr("src") != "/static/image/add.png") {
                var cur_dom = $(this);
                cur_dom.closest(".thumbnail").find("img").attr("src", "/static/image/add.png");
                pic_src = [];
                url_list_length = 0;
                $(".pic-display").find("img").each(function(k,v){
                    if($(v).attr("src")!="/static/image/add.png"){
