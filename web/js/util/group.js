/**
 * Created by GF on 2016/2/26.
 */
(function ($) {
    $.fn.gfGroup = function(options){
        var $element = $(this);
        var condition ={},
            tool_str = '<span class="operate-btns" style="display: none;">',
        ...
        var Group = {
            ...
            init:function(){
                ...
                if(ops.is_add || ops.is_edit || ops.is_delete){
                    ops.insert_target.on("mouseenter",".group_a",function(){ // 注意绑定事件的对象范围
                        $(this).parent().find(".operate-btns").show();
                    })
                    .on("mouseleave",".gf-category",function(){
                        $(this).find(".operate-btns").fadeOut(300);
                    });
                }
                if(ops.is_sync){
            ...
            select_group:function(){
                ...
                ops.insert_target.html('<div style="width: 32px; height: 32px; float:left">'+
                            ...
                            if(data.status == 1){
                                if(!data["json"]){
                                    if(ops.is_sync){
                                        ...
                                        if(ops.set_group_tip){
                                             ops.insert_target.append(ops.set_group_tip);
                                        }
                                    }
                                }else{
                                    ops.insert_target.empty().append(inher_group);
                                    Group.render_group(data["json"], "0");
                                }
                            }else{
                                if(ops.group_modal){
...
//                                ops.insert_target.empty().append(ops.fail_tip); // 之后加个判断 同步失败和初始化失败分开
                    ...
                    ops.insert_target.append('<div class="gf-global-tip"><div class="gf-tip-text"></div>' +
                                        '<a href="javascript:void(0)" class="gf-tip-close">&times;</a></div>');
                    Group.render_group(ops.group_data, "0");
                }else{
                ...
                for (var i=0; i<gl.length; i++){
                    ...
                    if(!child || child.length == 0){
                        ...
                        html = ops.is_leaf_href ? ("<div class='gf-category'"+
                            "style='margin-left: "+26*level+"px' data-pid='"+$pid+"' data-id='"+gl[i]["groupId"]+ "'data-name='"+ gl[i]["groupName"] +
                            "'><span class='group_a'>"+gl[i]["groupName"]+"</span>"+ $tool+ "</divtrue>")
                ...
                if(Number(group_count) < ops.limit-2){ // 未分组和所有分组不算在内
                    ...
                    if($category.nextAll(".gf-category").length == 0){
                        ...
                        $category.nextAll(".gf-category").each(function(k, v){
                            ...
                            if(kv_m <= m){
                                target_position = kv.prev(".gf-category");
                                return false
                            }
                        });
                    }
