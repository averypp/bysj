...
if ( ! class_exists('WPLA_StocksLogger') ) :
    ...
    class WPLA_StocksLogger {
        ...
        public function logProductStockUpdate( $product ) {
            ...
            $data['product_id'] = empty( $product->variation_id ) ? $product->id : $product->variation_id;
            $data['caller']     = $caller;
            $data['method']     = $method;
            $data['backtrace']  = WPLA_DEBUG > 5 ? $backtrace : '';
            $this->insertLogRecord( $data );
        ...
        protected function getFormattedBacktrace() {
            ...
            foreach ( $trace as $call ) {
                if ( !empty( $call['class'] ) ) {
                    $formatted .= sprintf( "%d. %s::%s() %s:%d\n", $loop, $call['class'], $call['function'], $call['file'], $call['line'] );
                } else {
                    $formatted .= sprintf( "%d. %s() %s:%d\n", $loop, $call['function'], $call['file'], $call['line'] );
                }
                $loop++;
            }
