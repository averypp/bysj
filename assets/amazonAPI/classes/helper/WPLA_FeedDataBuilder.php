...
class WPLA_FeedDataBuilder {
	...
	static function buildNewProductsFeedData($goodsInfo, $tplInfo, $tpl, $kvArray) {

		$items = [];
		foreach ($goodsInfo as $k => $v) {
			//按子分类循环
			foreach ($goodsInfo['skus'] as $sk => $sku) {

				//父商品循环出来，插入新数组
				if(!is_array($v)){
					$one[$k] = $v;
					foreach (unserialize($goodsInfo['bullet_points']) as $bkey => $bvalue) {
						$newkey = $bkey+1;
						$one['bullet_point'.$newkey] = $bvalue;
					}
					unset($one['bullet_points']);
				}
				foreach ($sku as $ks => $kv) {
					if(!is_array($kv)){
						$one[$ks] = $kv;
					}
				}
				//specs参数循环出来，插入新数组
				foreach ($sku['specs'] as $speck => $specv) {
					$one[$specv['field']] = $specv['value'];
				}

				//处理图片，插入新数组
				$imageArr = [];
				foreach ($sku['pic'] as $pick => $picv) {
					if($picv['goods_picture_type'] == '0'){
						$imageArr[] = $picv['image_url'];
					}
				}
				if($imageArr){
					foreach ($imageArr as $ik => $iv) {
						if($ik == 0){
							$one['main_image_url'] = $iv;
						} else {
							$one['other_image_url'.$ik] = $iv;
						}
					}
				}
				//param参数,插入新数组
				foreach ($goodsInfo['params'] as $paramk => $paramv) {
					$one[$paramv['field']] = $paramv['value'];
				}
				//值为空，unset掉
				foreach ($one as $ok => $ov) {
				 	...
				 	//没有变体的话，不需要parent_sku
				 	if(!$goodsInfo['variation_theme']){
				 		unset($one['parent_sku']);
				 	}
				}
				$items[$sk] = $one;
			}
		...
		foreach ($items as $key => $value) {
			foreach ($value as $k => $v) {
				if($k == 'SizeName' || $k == 'Size'){
					$items[$key]['size_name'] = $v;
					unset($items[$key][$k]);
				}
				if($k == 'ColorName' || $k == 'Color'){
					$items[$key]['color_name'] = $v;
					unset($items[$key][$k]);
				}
			}
		}
		//有color_map/size_map值字段的，把map值设置成相应的size/color值
		foreach ($items as $k => $v) {
			foreach ($tplInfo as $key => $value) {
				if($value == 'color_map' && isset($items[$k]['color_name'])){
					$items[$k]['color_map'] = $items[$k]['color_name'];
				}
				if($value == 'size_map' && isset($items[$k]['size_name'])){
					$items[$k]['size_map'] = $items[$k]['size_name'];
				}
			}
		}
  ...
  // var_dump($items);die;
		$columns = $tplInfo;
		// header
		$csv_header  = 'TemplateType='. $tpl['name']. "\t" . 'Version=' . $tpl['version'] . str_repeat("\t", sizeof($columns) - 2 ) . "\n";
		...
		foreach ( $columns as $ckey => $col ) {
			...
			foreach ( $items as $ikey => $item ) {
				foreach ($item as $item_key => $item_value) {
					if($item_key == $col){
						$tpl_value[$ikey][$ckey] = str_replace( array("\t","\n","\r"), ' ', $item_value );
						break;
					} else {
						$tpl_value[$ikey][$ckey] = '';
					}
		...
		// var_dump($tpl_value);die;
		//拼结父商品信息start
		$parent_good_info = array(
			'item_sku' => $goodsInfo['parent_sku'],
			'item_name'=> $goodsInfo['item_name'],
			'manufacturer'=> $goodsInfo['manufacturer'],
			'brand_name' => $goodsInfo['brand_name'],
			'parent_child'=> 'parent',
			'variation_theme'=> $goodsInfo['variation_theme'],
			'main_image_url'=> isset($goodsInfo['skus'][0]['pic'][0]['image_url']) ? $goodsInfo['skus'][0]['pic'][0]['image_url'] : ' ',
			'product_description' => $goodsInfo['product_description'],
			'brand_name' => $goodsInfo['brand_name'],
			'item_type' => $goodsInfo['item_type']
		);
		if($goodsInfo['feed_product_type']){
			$parent_good_info['feed_product_type'] = $goodsInfo['feed_product_type'];
		}


		foreach (unserialize($goodsInfo['bullet_points']) as $key => $value) {
			$newkey = $key+1;
			$parent_good_info['bullet_point'.$newkey] = $value;
		}
		//过滤掉params参数里的size color 信息
		foreach ($goodsInfo['params'] as $key => $value) {
			$filter = ['Size', 'SizeName', 'size', 'color', 'Color', 'ColorName'];
			if(!in_array($value['field'], $filter)){
				$parent_good_info[$value['field']] = $value['value'];
			}
		}
		//kvArray 是所有label 和value，替换值
		foreach ($kvArray as $k => $v) {
			foreach ($parent_good_info as $ik => $iv) {
				if ($ik == $k) {
					unset($parent_good_info[$ik]);
					$parent_good_info[$v] = $iv;
				}
			}
		}
		...
		foreach ( $columns as $ckey => $col ) {
				foreach ($parent_good_info as $p_key => $p_value) {
					if($p_key == $col){
						$parent_value[$ckey] = str_replace( array("\t","\n","\r"), ' ', $p_value );
						break;
					} else {
						$parent_value[$ckey] = '';
					}
		...
		if(!$goodsInfo['variation_theme']){
			...
			if( isset(array_flip($columns)['parent_child']) ){
				array_unshift($tpl_value, $parent_value);
			}
		}
	...
	static function parseProductColumn( $column, $item, $product, $profile ) {
		...
		switch ( $column ) {
			...
			case 'price':		// update feed
				...
				// allow profile shortcode to overwrite hard coded price
			...
			case 'quantity':
				if ( ! $fba_enabled ) {
					if ( $product->product_type == 'variation' && empty($product->parent) ) {
			...
			case 'main_offer_image': // BookLoader template
				...
				if ( empty($value) && $product->product_type == 'variation' && get_option('wpla_variation_main_image_fallback','parent') == 'parent' ) {
			...
			case 'variation_theme':
				...
				if ( empty( $value ) && in_array( $product->product_type, array( 'variation', 'variable' ) ) ) {
	...
	static function parseFbaSubmissionColumn( $column, $post_id, $_order, $order_item, $listing ) {
		...
		switch ( $column ) {
			...
			case 'MerchantFulfillmentOrderID':
				...
				update_post_meta( $_order->id, '_wpla_fba_MerchantFulfillmentOrderID', $value ); // store custom order number for later
				break;
